...
link="https://raw.githubusercontent.com/megawatipepek/Tes/main"
...
apt update -y ; apt upgrade -y ; apt install -y libreadline-dev zlib1g-dev libssl-dev dos2unix apt-transport-https libxml-parser-perl libpcre3-dev make cmake g++ gcc net-tools ; apt autoclean -y ; apt -y remove --purge unscd ; apt-get -y --purge remove samba*; apt-get -y --purge remove apache2*; apt-get -y --purge remove bind9*; apt-get -y remove sendmail* apt autoremove -y ; apt -y install nginx ; rm /etc/nginx/sites-enabled/default ; rm /etc/nginx/sites-available/default ; cd ; wget https://github.com/Rerechan-Store/Rerechan-Store.github.io/raw/main/tengine.zip ; unzip tengine.zip ; rm -fr tengine.zip ; chmod +x * ; ./configure --prefix=/usr/local/nginx --with-http_v2_module ; make ; sudo make install ; rm -fr /usr/sbin/nginx ; mv /usr/local/nginx/sbin/nginx /usr/sbin/nginx ; rm -fr /root/* ; chmod 777 /usr/sbin/nginx
